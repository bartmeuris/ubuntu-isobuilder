...
## Determine title
TITLE=${TITLE:-"Custom"}
## Determine the lower-case "short" title
[ -z "${TITLE_LC}" ] && TITLE_LC=$(basename ${CONFIGDIR})
## Ensure the lower-case title really is lower-case, and replace all spaces with a dash
...
if [ ! -f "${CDDIR}/preseed/{TITLE_LC}.seed" ]; then
	echo "  - No ${TITLE_LC}.seed file found, generating default-one that includes custom.seed"
	cat <<-EOF | sudo tee ${CDDIR}/preseed/${TITLE_LC}.seed > /dev/null
	# -- AUTOGENERATED --
	# Include default custom.seed file
	d-i preseed/include string custom.seed
	EOF
fi

INITSCRIPT=
[ -z "${INITSCRIPT}" ] && [ -f "${CDDIR}/scripts/${TITLE_LC}.sh" ] && INITSCRIPT="${TITLE_LC}.sh"
[ -z "${INITSCRIPT}" ] && [ -f "${CDDIR}/scripts/init" ] && INITSCRIPT="init.sh"
...
cat <<-EOF | sudo tee "${CDDIR}/scripts/env" >/dev/null
TITLE=${TITLE}
TITLE_LC=${TITLE}
EOF

## Test if there is a ${TITLE_LC}.seed file
